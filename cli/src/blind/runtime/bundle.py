...
_PINNED_SIGNING_KEY_HEX = "7e998cc937c394810eb00702aa9b6fabf1e70adcc2cd91f7bca953b7c312dd36"
...
def active_signing_key_hex(override: str | None = None) -> str:
    ...
    if chosen != _PINNED_SIGNING_KEY_HEX and not _warned_nondefault_key:
        ...
        print(
            ...
            "(from $BLIND_SIGNING_KEY / override) — dev / self-host only.",
            file=sys.stderr,
        )
    return chosen
    ...
    with tarfile.open(fileobj=io.BytesIO(tar_bytes), mode="r:*") as tf:
        ...
        for member in members:
            ...
            if member.issym() or member.islnk():
                raise VerificationError(f"Refusing symlink in bundle tar: {name!r}")
            if member.isfile():
                total += member.size
                if total > max_bytes:
                    raise VerificationError(
                        f"Bundle expands past the {max_bytes}-byte cap — refusing to extract")
        # If the tar has a single top-level dir, flatten it into dest.
