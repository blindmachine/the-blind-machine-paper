...
def _iter_bundle_files(root: Path, exclude: frozenset[str]) -> list[tuple[str, Path]]:
    ...
    suffix is compiled Python (`.pyc`/`.pyo`)."""
    ...
    for p in sorted(root.rglob("*")):
        ...
        if rel_path.suffix in EXCLUDED_BUNDLE_SUFFIXES:
            continue
        out.append((rel_path.as_posix(), p))
...
def canonical_bundle_digest(
    ...
    for rel, abspath in _iter_bundle_files(root, exclude):
