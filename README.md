...
Snapshot assembled: 2026-07-11T03:49:31Z
...
| **Tier 2 — full E1–E8** | the paper's tables, end to end | this repo + `uv` + `python3`; E5–E8 additionally need `curl`, `bcftools`, `tabix` |

Tier 2 is what this package delivers. E1–E4 are fully self-contained
(synthetic cohorts, seeded, no network). E5–E8 are **aggregate-reproducible**
from public IGSR 1000 Genomes data via `fetch_public_data.sh` — no human
genotypes are shipped here.
...
# 2. Run the core synthetic experiments (E1 + E4; add `full` for E2 + E3)
bash experiments/run_all.sh          # fast profile   ~2-4 min
bash experiments/run_all.sh full     # full  profile  ~15-25 min

# 3. Verify the results against the committed oracle
python3 experiments/verify.py        # prints RESULT: PASS on success
```

On success `run_all.sh` prints `RESULT: PASS` and writes paper-facing CSV
tables to `experiments/results/`. Any bit-exactness, taxonomy, or differencing
regression prints `RESULT: FAIL` and exits non-zero.

### Real-human-DNA studies (E5–E8, optional)

These fetch a bounded chr22 window from the 1000 Genomes Project — no human
genotypes live in this repo.
...
bash experiments/e8_public_ld_window.sh
```

Requirements: `curl`, `bcftools`, `tabix`. The download lands under the
...
the-blind-machine-paper/
  README.md            # this file
  LICENSE              # MIT
  applications/        # the 6 signed application bundles (Ed25519 sigs, content-addressed)
  experiments/         # e1..e8, run_all.sh, verify.py, lib.sh, fetch_public_data.sh, DATA_SOURCES.md
