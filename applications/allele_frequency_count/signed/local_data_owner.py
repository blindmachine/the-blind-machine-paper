...
def encode(raw: list[int | None], length: int) -> list[int]:
    ...
    For `allele_frequency_count` the encoding is deliberately minimal (the
    simplest possible circuit): the raw input is an alt-allele dosage vector
    ``g in {0,1,2}^L`` over the published coordinate definition. Missing calls
    (``None``) encode as 0; every present call is validated in {0,1,2}; the vector
    is zero-padded to length L (rejected if longer).

    The append-1 sentinel is NOT added here; it is appended at encryption time,
    matching docs/spec.md ("encryption appends a sentinel slot").
