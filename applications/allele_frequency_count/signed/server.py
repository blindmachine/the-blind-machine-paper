#!/usr/bin/env python3
"""server.py — the BLIND computation for `allele_frequency_count`.
...
cannot drift from this encrypted path):
    ...
    aggregate(inputs, E):          # E provides zero / add
        acc = <first input>
        for ct in rest(inputs):    # allele_frequency_count: pure coordinate-wise add
            acc = E.add(acc, ct)
        return acc

Additive-only: the append-1 sentinel rides along, so ``sum_i (g_i || 1)`` yields
the per-coordinate allele-count vector in the first L slots and the exact
