#!/usr/bin/env python3
"""server.py — the BLIND computation for `cohort_histogram`.
...
the server never sees a single plaintext bucket membership.

The fold is written ONCE against an abstract evaluator ``E`` (the cleartext oracle
in docs/simulation_mode.md swaps a plaintext evaluator into the same shape so it
cannot drift from this encrypted path):

    aggregate(inputs, E):          # E provides zero / add
        acc = <first input>
        for ct in rest(inputs):    # cohort_histogram: pure coordinate-wise add
            acc = E.add(acc, ct)
        return acc

Additive-only: the sentinel slot rides along, so ``sum_i (h_i || 1)`` yields the
per-bucket count vector in the first B slots and the exact contributor count N in
the trailing sentinel slot. Because each ``h_i`` is one-hot, the first B slots
themselves also sum to N — a redundancy stage 50 asserts.
